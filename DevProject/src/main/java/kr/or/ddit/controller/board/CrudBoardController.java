...
public class CrudBoardController {
	...
	// 빈이 등록되고 초기화단계에서 바로확인할때 사용
	@PostConstruct
	public void init() {
		log.info("aopProxy상태 (interface기반) : {}" + AopUtils.isAopProxy(service));
		log.info("aopProxy상태 (class기반) : {}" + AopUtils.isCglibProxy(service));
	...
	public String crudRead(Model model, int boardNo) {
	...
	public String crudModifyForm(Model model, int boardNo) {
