...
public class BoardController {
	 ...
	 * 1. 요청 경로 매핑
	 * 
	 * @RequestMapping 의value속성에 요청 경로를 설정한다.
	 * 
	 * - 요청경로는 반드시 설정해야하는 필수 정보이다. 
	 * - 속성이 하나일때는 속성명이 생략 될 수 있다. 
	 * - 컨트롤러의 클래스 레벨과 메서드 레벨로 지정할 수있다. 
	 * - 클래스 레벨로 요청 경로를 지정하면 메소드 레벨에서 지정한 경로의 기본경로가 취급된다.
	 *  - 클래스 레벨의 요청경로에 메소드레벨의 요청령로를 덧붙이 형태가 최종 경로가된다.
	 */
	 ...
	 * 
	 * - 해당 경로로 요청시, board폴더에 read.jsp를 찾아서 페이지를 출력해주는거 같지만, 
	 * 경로상으로만 보면
	 * board/read/boardNo라는 파일까지 찾게된다.
	 * 
	...
	public String rea(@PathVariable("boardNo") int boardNo) {
		logger.info("read() 실행....!");
	    ...
	    * 화면으로 응답하는 경우에는 HTTP 메서드로 GET방식과 POST 방식 두가지를 사용할 수 있다
	    */
	 ...
	 *  버튼이나 링크에 따라 호출할 메서드를 바꿔야 할대 사용한다.
	 * 
	 * 확인페이지 : views/board/list,modify,register,read 등
	 */
	...
	public String removeFormParamsGet() {
		...
		logger.info("removeFormParamsGer()실행....!");
		return "board/remove";
	}
	...
	/*
	 * 5. Header 매핑 - 요청 헤더를 매핑조건으로 지정하는 경우에는 headers 속성을 사용한다.
	 */
	...
	// Headers매핑이지만, headers설정을 하지않고 요청
	...
	/*
	 * 6. contentType 매핑 - 요청의 contentType 헤더 값을 매핑 조건으로 지정하는 경우에는 consumes속성을 사용한다.
	 * 
	 */

	// 응답상태코드와 값을 제네릭에 설정되어있는 타입에 따라서 내마음대로 내보낼 수 있다.
	// 응답상태 코드 : HttpStatus라는 객체 안에있는 여러상태코드를 선택해서 내보낼 수 있다.
	// 결과값 : 제네릭타입에 설정되어있는 해당타입의 값으로 결과의 값을 설정해서 내보낼 수 있다.

	// consumes 속성값을 지정하지않고 테스트
	// consumes 속성값을 지정하지않으면 기본값으로 "application/json" 미디어타입으로 지정된다.
	@RequestMapping(value = "/{boardNo}", method = RequestMethod.POST)
	public ResponseEntity<String> modifyContentType(@PathVariable("boardNo") int boardNo, Board board) {
	...
	public ResponseEntity<String> modifycontentTypeByXml(@PathVariable("boardNo") int boardNo, Board board) {
	...
	/*
	 * 7. Accept 매핑 사용 -process 속성값을 지정해서요청하는 미디어타입을 처리한다. /process 속성값을 지정하지않으면
	 * 기본값인 "application/json" 미디어타입으로 지정된다.
	 */

	// produces 속성값을 지정하지않고 테스트
