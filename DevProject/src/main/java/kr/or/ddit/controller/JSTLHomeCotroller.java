...
@RequestMapping("/jstl")
@Slf4j
public class JSTLHomeCotroller {
		/*
		 * 5. 표준 태그 라이브러리(JSTL)
		 *  - 많은 개발자들이 JSP에서 코드를 깔끔하게 작성하기 위해서 커스텀태그를 만들어왔는데,
		 *  이런중복되는 노력을 없애기 위해서 나온것이 바로 JSTL이다.
		 *  
		 *   1)core태그 라이브러리
		 *   
		 *   	요소		|		설명
		 ...
		 *  <c:catch>		|	예외를 처리한다.
		 *  <c:if>			|	조건을 지정하고 지정된 조건과 일치하는 처리내용을 구현한다.
		 *  <c:choose>		|	여러 조건을 지정할때 사용한다.
		 *  <c:when>		|	여러조건을 지정하고 지정된 조건과 일치하는 처리내용을 구현한다.
		 *  <c:otherwise>	|	<c:when>요소에서 지정한 조건에 모두 일치하지않을때 처리할 내용을 구현한다.
		 *  <c:foreach>		|	컬렉션이나 배열의 각항목을 처리할때 사용한다.
		 *  <c:fortokens>	| 구분자로 구분된 각각의 토큰을 처리할 때 사용한다.
		 *  <c:import>		| URL을 사용하여 다른자원을 삽입한디.
		 *  <c:url	>		| URL을 작성한다.
		 ...
		 *   	요소			|		설명
		 *  ---------------------------------------------------
		 *  <fn:contails>			 |	지정한 문자열이 포함되있는지 판단한다.
		 *  <fn:containsIgnorecCase> |	지정한 문자열이 대문자/소문자를 구분하지않고 포함돼있는지 판단한다.
		 *  <fn:startsWith> 		 | 지정한 문자열로 시작하는지 판단한다.
		 *  <fn:endsWith> 		     | 지정한 문자열로 끝나는지 판단한다.
		 *  <fn:indextOf> 		     | 지정한 문자열이 처음으로 나왔을때의 인덱스를 구한다.
		 *  <fn:length>			     | 컬렉션 또는 배열의 요소개수, 문자열의 길이를 구한다.
		 *  <fn:escapeXml>		     |	지정한 문자열을 XML 구문으로 해석되지않도록 이스케이프한다.
		 *  <fn: replace>		     | 문자열을 치환한다.
		 *  <fn:toLowerCase>	     | 문자열을 소문자로 변환한다.
		 *  <fn:toUpperCase>	     | 문자열을 대문자로 변환한다.
		 *  <fn:trim>			     | 문자열을 trim한다.
		 *  <fn:substring>		     | 지정한 범위에 해당하는 문자열을 잘라낸다.
		 *  <fn:substringAfter>	     |	지정한 문자열엘 일치하는 이후의 문자열을 잘라낸다.
		 *  <fn:substringBefor>	     |	지정한 문자열엘 일치하는 이전의 문자열을 잘라낸다.
		 ...
		 * ------------------------------------------------
		 * value	|	Object		|	출력할 값
		 * escapeXml|	boolean		|	특수문자를 변환할지 여부
		 * default  |	Object		|	value의 결과값이 null인경우 출력할 값
		 *  
		 *  
		 */
		...
		public String hom0103(Model model) {
			Member member = new Member();
		...
		//JSP에서 사용될 변수 memberID를 설정하여 사용하고, 태그의 몸체를 값으로 사용한다.
